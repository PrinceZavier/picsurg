...
**Status:** Updated for Phase 7 (Analytics)
...
| Analytics Events | Non-sensitive | TelemetryDeck servers | None (no PHI) |
...
|-------------------|------------|-------|
| Business Associate Agreement | N/A | No third parties handle PHI. TelemetryDeck receives only anonymous event data (no PHI) |
...
- Analytics are anonymous — no PHI, no PII, auto-hashed device IDs
- Network requests limited to TelemetryDeck analytics signals (no photo data transmitted)
...
TelemetryDeck receives **only** the following (never PHI):

| Data Sent | Example | PHI? |
|-----------|---------|------|
| Event name | `scan_completed` | No |
| Counts | `photosScanned: 15` | No |
| Durations | `durationSeconds: 8` | No |
| Success/failure | `success: true` | No |
| Feature toggles | `biometricEnabled: true` | No |
| Hashed device ID | Auto-generated by SDK | No |

**Never sent**: photo content, filenames, patient data, location, user identifiers, email addresses.
